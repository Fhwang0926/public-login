...
async function commandExists(name) {
  const result = await runCommand("sh", ["-c", `command -v ${name} >/dev/null 2>&1`], "/");
  return result.ok;
}
...
async function runAptGet(args) {
  const env = { ...process.env, DEBIAN_FRONTEND: "noninteractive" };
  let result = await runCommand("apt-get", args, "/", { env });
  if (result.ok) {
    return result;
  }
  ...
  const needsElevated =
    result.code === "EACCES" ||
    /permission denied|are you root|not allowed to/i.test(result.stderr);
  if (!needsElevated || !(await commandExists("sudo"))) {
    return result;
  }

  return runCommand("sudo", ["apt-get", ...args], "/", { env });
}

async function installGitViaApt() {
  if (process.platform !== "linux") {
    return {
      ok: false,
      error: "git이 없습니다. apt 자동 설치는 Linux에서만 지원합니다.",
    };
  }
  if (!(await commandExists("apt-get"))) {
    return {
      ok: false,
      error: "git이 없고 apt-get도 찾을 수 없습니다.",
    };
  }

  const update = await runAptGet(["update", "-qq"]);
  if (!update.ok) {
    return { ok: false, error: update.stderr || "apt-get update 실패" };
  }

  const install = await runAptGet(["install", "-y", "-qq", "git"]);
  if (!install.ok) {
    return { ok: false, error: install.stderr || "apt-get install git 실패" };
  }
...
async function ensureGitAvailable() {
  ...
  const install = await installGitViaApt();
  if (!install.ok) {
  ...
  if (!(await isGitAvailable())) {
    return {
      ok: false,
      error: "apt로 git 설치 후에도 git 명령을 실행할 수 없습니다.",
    };
  }
...
async function getUpdateStatus(repoDir) {
  ...
  const gitReady = await ensureGitAvailable();
  ...
  if (!isRepo) {
    return {
      ...
      canPull: false,
      hasRemoteUpdates: false,
      gitInstalled: gitReady.installed,
      message: gitReady.installed
        ? "git 설치 완료. Git 저장소가 아닙니다."
        : "Git 저장소가 아닙니다.",
    };
  }
  ...
  const canPull = enabled && hasUpstream && !dirty && !fetchError;
  const hasRemoteUpdates = behind > 0;
  ...
  if (!enabled) {
    ...
    message = gitReady.installed ? "apt로 git을 설치했습니다. 이미 최신 커밋입니다." : "이미 최신 커밋입니다.";
  } else {
    message = gitReady.installed
      ? `apt로 git을 설치했습니다. 원격보다 ${behind}커밋 뒤처져 있습니다.`
      : `원격보다 ${behind}커밋 뒤처져 있습니다.`;
  }
...
async function performSelfUpdate(repoDir) {
  ...
  const gitReady = await ensureGitAvailable();
  if (!gitReady.ok) {
    throw new Error(gitReady.error);
  }
  ...
  return {
    pulled: true,
    message: gitReady.installed
      ? "apt로 git을 설치하고 업데이트를 적용했습니다. 서버를 재시작합니다."
      : "업데이트를 적용했습니다. 서버를 재시작합니다.",
    npmInstall,
