...
/** req.body / form 필드 → 세션에 저장할 overrides 객체 */
function parseBioPassOverridesFromBody(body) {
  ...
  for (const field of BIO_PASS_OVERRIDE_FIELDS) {
    ...
    if (value) overrides[field.key] = value;
  }
  return overrides;
}

function getBioPassConfigFromRequest(req) {
  return getBioPassConfig(req.session && req.session.bioPassOverrides);
}
