...
/** 세션/폼에서 오버라이드 가능한 Bio-Pass 필드 */
const BIO_PASS_OVERRIDE_FIELDS = [
  { key: "apiBase", envKey: "BIO_PASS_API", label: "BIO_PASS_API" },
  { key: "clientId", envKey: "BIO_PASS_CLIENT_ID", label: "BIO_PASS_CLIENT_ID" },
  {
    key: "clientSecret",
    envKey: "BIO_PASS_CLIENT_SECRET",
    label: "BIO_PASS_CLIENT_SECRET",
  },
  {
    key: "redirectUri",
    envKey: "BIO_PASS_REDIRECT_URI",
    label: "BIO_PASS_REDIRECT_URI",
  },
  {
    key: "oauthState",
    envKey: "BIO_PASS_OAUTH_STATE",
    label: "BIO_PASS_OAUTH_STATE",
  },
];

/**
 * @param {object} [overrides] 세션 등에서 온 값. 빈 문자열은 오버라이드하지 않음.
 */
function getBioPassConfig(overrides = {}) {
  const port = Number(process.env.PORT) || 5000;
  const o = overrides && typeof overrides === "object" ? overrides : {};

  const pick = (overrideVal, envVal, fallback = "") => {
  ...
  const apiBase = trimSlash(
    pick(o.apiBase, process.env.BIO_PASS_API, "http://127.0.0.1:3030/api")
  );
  const clientId = pick(o.clientId, process.env.BIO_PASS_CLIENT_ID);
  const clientSecret = pick(o.clientSecret, process.env.BIO_PASS_CLIENT_SECRET);
  const redirectUri =
    pick(o.redirectUri, process.env.BIO_PASS_REDIRECT_URI) ||
    `http://127.0.0.1:${port}/auth/callback`;
  const scope =
    String(process.env.BIO_PASS_SCOPE || "email,phone").trim() || "email,phone";
  const oauthState = pick(o.oauthState, process.env.BIO_PASS_OAUTH_STATE);

  const overriddenKeys = BIO_PASS_OVERRIDE_FIELDS.filter(
    (f) => o[f.key] !== undefined && o[f.key] !== null && String(o[f.key]).trim() !== ""
  ).map((f) => f.key);
...
/** req.body / form 필드 → 저장할 overrides 객체 */
function parseBioPassOverridesFromBody(body) {
  return sanitizeOverrideValues(body);
}

function sanitizeOverrideValues(raw) {
  const src = raw && typeof raw === "object" ? raw : {};
  const overrides = {};
  for (const field of BIO_PASS_OVERRIDE_FIELDS) {
    const value = src[field.key];
    if (value === undefined || value === null) continue;
    const trimmed = String(value).trim();
    if (trimmed) overrides[field.key] = trimmed;
  }
  return overrides;
}

let overrideStorePath = null;
let storedOverrides = {};

function initBioPassOverrideStore(filePath) {
  overrideStorePath = filePath;
  storedOverrides = {};
  try {
    if (filePath && fs.existsSync(filePath)) {
      storedOverrides = sanitizeOverrideValues(
        JSON.parse(fs.readFileSync(filePath, "utf8"))
      );
    }
  } catch (err) {
    console.error("[biopass] override store read failed:", err.message);
    storedOverrides = {};
  }
}

function persistOverrideStore() {
  if (!overrideStorePath) return;
  fs.mkdirSync(path.dirname(overrideStorePath), { recursive: true });
  if (Object.keys(storedOverrides).length === 0) {
    if (fs.existsSync(overrideStorePath)) {
      fs.unlinkSync(overrideStorePath);
    }
    return;
  }
  fs.writeFileSync(
    overrideStorePath,
    `${JSON.stringify(storedOverrides, null, 2)}\n`,
    "utf8"
  );
}

function getStoredBioPassOverrides() {
  return storedOverrides;
}

function setStoredBioPassOverrides(overrides) {
  storedOverrides = sanitizeOverrideValues(overrides);
  persistOverrideStore();
  return storedOverrides;
}

function clearStoredBioPassOverrides() {
  storedOverrides = {};
  persistOverrideStore();
}

function getBioPassConfigFromRequest() {
  return getBioPassConfig(getStoredBioPassOverrides());
}
